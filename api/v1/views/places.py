...
def get_place(place_id):
    ...
    place_obj = [obj.to_dict() for obj in stored_places if obj.id == place_id]
    if place_obj == []:
        abort(404)
...
def delete_place(place_id):
    ...
    stored_places = storage.all("Place").values()
    place_obj = [obj.to_dict() for obj in stored_places
                 if obj.id == place_id]
    if place_obj == []:
        abort(404)
...
def create_place(city_id):
    ...
    stored_cities = storage.all("City").values()
    city_obj = [obj.to_dict() for obj in stored_cities
                if obj.id == city_id]
    if city_obj == []:
        abort(404)
    places = []
    ...
    user_obj = [obj.to_dict() for obj in stored_users
